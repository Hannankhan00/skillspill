...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   S K I L L S P I L L  â€”  R E C R U I T E R  P R O F I L E
...
const profileData = {
    ...
    tagline: "Talent Scout â€¢ Engineering Recruitment Lead",
    ...
    activeBounties: [
        ...
        {
            title: "ML Engineer â€” Computer Vision",
            budget: "$18,000",
            applicants: 15,
            daysLeft: 20,
    ...
    recentPosts: [
        ...
        {
            id: 2,
            content: "Just closed our Full-Stack Lead position in record time â€” 8 days from posting to hire! SkillSpill's skills-first approach made it incredibly easy to find the right candidate. ðŸ’œ",
            likes: 234,
            comments: 42,
            time: "2d",
        },
        {
            id: 3,
            content: "Hot take: The best engineering hires I've made weren't from the most prestigious companies â€” they were from open-source contributors with deep domain expertise. Skills > Pedigree.",
            likes: 567,
            comments: 93,
            time: "5d",
        },
                    ...
                    {tabs.map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)}
                            className={`px-4 sm:px-6 py-3 text-[12px] sm:text-[13px] font-semibold border-b-2 transition-all cursor-pointer bg-transparent whitespace-nowrap
                                ${activeTab === tab ? "border-purple-500 text-purple-600" : "border-transparent text-[var(--theme-text-muted)] hover:text-[var(--theme-text-tertiary)]"}`}>
                            {tab}
                        </button>
                    ))}
                </div>
                        ...
                        <>
                            ...
                            <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] shadow-sm p-4 sm:p-5">
                                ...
                                <div className="flex items-center gap-3 mb-3">
                                    ...
                                    <div>
                                        <p className="text-[13px] font-bold text-[var(--theme-text-primary)]">{profileData.companyName}</p>
                                        <p className="text-[11px] text-[var(--theme-text-muted)]">{profileData.companyInfo.industry}</p>
                                    </div>
                                </div>
                            ...
                            {/* Skill Demand */}
                            <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] shadow-sm p-4 sm:p-5">
                                <h2 className="text-[14px] font-bold text-[var(--theme-text-primary)] mb-3">ðŸ“Š Skills In Demand</h2>
                                        ...
                                        <div key={skill.name}>
                                            <div className="flex items-center justify-between mb-1">
                                                <span className="text-[12px] font-medium text-[var(--theme-text-secondary)] flex items-center gap-1.5">
                                                    {skill.name}
                                                    {skill.trending && <span className="text-[8px] px-1.5 py-0.5 rounded-full bg-orange-100 text-orange-600 font-bold">ðŸ”¥ HOT</span>}
                                                </span>
                            ...
                            <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] shadow-sm p-4 sm:p-5">
                                <h2 className="text-[14px] font-bold text-[var(--theme-text-primary)] mb-3">ðŸŽ¯ Recent Successful Hires</h2>
                                <div className="space-y-2.5">
                                    {profileData.recentHires.slice(0, 3).map(hire => (
                                        <div key={hire.name} className="flex items-center gap-3 p-2.5 rounded-xl bg-[var(--theme-bg-secondary)] hover:bg-purple-50 transition-colors">
                    ...
                    {activeTab === "Bounties" && (
                        <div className="space-y-4">
                            {profileData.activeBounties.map((bounty, i) => (
                                <div key={i} className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] shadow-sm p-4 sm:p-5 hover:border-purple-300 hover:shadow-md transition-all">
                                ...
                                <div key={post.id} className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] shadow-sm p-4 sm:p-5">
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-[var(--theme-text-muted)]">{post.time} ago â€¢ {profileData.companyName}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-6 mt-3 pt-3 border-t border-[var(--theme-border-light)]">
                                        <span className="text-[11px] text-[var(--theme-text-muted)] flex items-center gap-1">â¤ï¸ {post.likes}</span>
                                        <span className="text-[11px] text-[var(--theme-text-muted)] flex items-center gap-1">ðŸ’¬ {post.comments}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] shadow-sm p-4 sm:p-5">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                                    {[
                                        { label: "Total Hires", value: "47", icon: "ðŸ†", bg: "bg-purple-50" },
                                        { label: "Avg Match", value: "92%", icon: "ðŸŽ¯", bg: "bg-green-50" },
                                        { label: "Time to Hire", value: "11d", icon: "âš¡", bg: "bg-orange-50" },
                                        { label: "Retention", value: "95%", icon: "ðŸ’Ž", bg: "bg-blue-50" },
                                    ].map(card => (
                            ...
                            <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] shadow-sm p-4 sm:p-5">
                                ...
                                <div className="space-y-2.5">
                                    {profileData.recentHires.map(hire => (
                                        <div key={hire.name} className="flex items-center gap-3 p-3 rounded-xl bg-[var(--theme-bg-secondary)] hover:bg-purple-50 transition-colors">
