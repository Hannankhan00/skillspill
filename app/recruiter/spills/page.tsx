            ...
            <div className="max-w-[800px] mx-auto px-4 sm:px-6 py-5 pb-20 lg:pb-8">
                ...
                <div className="flex items-center justify-between mb-5">
                    <div>
                        ...
                        <p className="text-[12px] text-gray-400 mt-0.5">Share updates, bounties, and hiring insights</p>
                    </div>
                    <button
                    ...
                    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm p-4 sm:p-5 mb-5">
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-[13px] font-bold text-gray-800">Recruiter</p>
                                <p className="text-[10px] text-gray-400">TechForge Solutions</p>
                            </div>
                        </div>
                            ...
                            {([["update", "📢 Update"], ["bounty", "💼 Bounty"], ["thought", "💭 Thought"]] as const).map(([type, label]) => (
                                <button key={type} onClick={() => setPostType(type)}
                                    className={`px-3 py-1.5 rounded-lg text-[10px] font-semibold border cursor-pointer transition-all
                                        ${postType === type ? "bg-purple-50 border-purple-200 text-purple-700" : "bg-white border-gray-200 text-gray-500 hover:bg-gray-50"}`}>
                                    {label}
                                </button>
                            ))}
                        </div>
                        ...
                        <textarea
                            ...
                            className="w-full min-h-[100px] p-3 rounded-xl bg-gray-50 border border-gray-200 text-[13px] text-gray-700 placeholder:text-gray-400 resize-none outline-none focus:border-purple-300 focus:ring-2 focus:ring-purple-50 transition-all"
                        />
                        ...
                        {showCode && (
                            <textarea
                                ...
                                className="w-full min-h-[100px] p-3 mt-2 rounded-xl bg-gray-900 border border-gray-700 text-[12px] text-purple-400 placeholder:text-gray-600 font-mono resize-none outline-none focus:border-purple-500 transition-all"
                            />
                        )}
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => setShowCode(!showCode)}
                                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium border cursor-pointer transition-all
                                        ${showCode ? "bg-purple-50 border-purple-200 text-purple-700" : "bg-white border-gray-200 text-gray-500 hover:bg-gray-50"}`}>
                                ...
                                <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium border border-gray-200 bg-white text-gray-500 hover:bg-gray-50 cursor-pointer transition-all">
                        ...
                        return (
                            <button key={filter} onClick={() => setActiveFilter(filter)}
                                className={`px-4 sm:px-5 py-2.5 text-[12px] font-semibold border-b-2 transition-all cursor-pointer bg-transparent flex items-center gap-1.5
                                    ${activeFilter === filter ? "border-purple-500 text-purple-600" : "border-transparent text-gray-400 hover:text-gray-600"}`}>
                                {filter}
                                <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold
                                    ${activeFilter === filter ? "bg-purple-100 text-purple-700" : "bg-gray-100 text-gray-500"}`}>
                                    {count}
                                </span>
                            </button>
                        );
                    ...
                    {filteredSpills.map(spill => (
                        <article key={spill.id} className={`rounded-2xl border bg-white shadow-sm overflow-hidden transition-all hover:shadow-md
                            ...
                            <div className="p-4 sm:p-5">
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-gray-400">{spill.time} • TechForge Solutions</p>
                                        </div>
                                    </div>
                                ...
                                {/* Content */}
                                <p className="text-[13px] text-gray-700 leading-relaxed mb-3 whitespace-pre-line">{spill.content}</p>
                                ...
                                {spill.bountyInfo && (
                                    <div className="rounded-xl border border-purple-200 bg-gradient-to-r from-purple-50 to-violet-50 p-3.5 mb-3">
                                        <div className="flex items-start justify-between">
                                            <div>
                                                <p className="text-[13px] font-bold text-gray-800">{spill.bountyInfo.title}</p>
                                ...
                                {spill.status === "published" && (
                                    <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            <button className="text-[10px] font-medium text-gray-400 hover:text-purple-600 cursor-pointer bg-transparent border-none transition-colors flex items-center gap-1">
                                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                                                Edit
                                            </button>
                ...
                {filteredSpills.length === 0 && (
                    <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-8 text-center">
                        ...
                        <p className="text-[12px] text-gray-400 mb-4">Start sharing bounties, hiring updates, and insights</p>
