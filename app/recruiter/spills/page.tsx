                ...
                {composerOpen && (
                    <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] shadow-sm p-4 sm:p-5 mb-5">
                        ...
                        <textarea
                            ...
                            className="w-full min-h-[100px] p-3 rounded-xl bg-[var(--theme-bg-secondary)] border border-[var(--theme-border)] text-[13px] text-[var(--theme-text-secondary)] placeholder:text-[var(--theme-text-muted)] resize-none outline-none focus:border-[#A855F7]/40 focus:ring-2 focus:ring-purple-50 transition-all"
                        />
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => setShowCode(!showCode)}
                                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium border cursor-pointer transition-all
                                        ${showCode ? "bg-[#A855F7]/10 border-[#A855F7]/30 text-[#A855F7]" : "bg-[var(--theme-card)] border-[var(--theme-border)] text-[var(--theme-text-muted)] hover:bg-[var(--theme-bg-secondary)]"}`}>
                ...
                <div className="space-y-4">
                    {filteredSpills.map(spill => (
                        <article key={spill.id} className={`rounded-2xl border bg-[var(--theme-card)] shadow-sm overflow-hidden transition-all hover:shadow-md
                            ...
                            {/* Draft badge */}
                            {spill.status === "draft" && (
                                <div className="px-4 py-1.5 bg-[var(--theme-surface)] border-b border-[var(--theme-border)] flex items-center justify-between">
                                    <span className="text-[10px] font-bold text-amber-500 flex items-center gap-1">
                                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                                        DRAFT
                                    </span>
                                    <button className="text-[10px] font-medium text-amber-500 hover:text-amber-400 cursor-pointer bg-transparent border-none flex items-center gap-1">
                                        Continue Editing <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14m-7-7 7 7-7 7" /></svg>
                                    </button>
                                </div>
                            )}
                            ...
                            <div className="p-4 sm:p-5">
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <div className="flex items-center gap-1">
                                        {spill.status === "published" && (
                                            <span className="text-[9px] font-medium px-2 py-0.5 rounded-full bg-green-500/10 text-green-500 border border-green-500/20 hidden sm:inline-block">Published</span>
                                        )}
                                    ...
                                    <div className="rounded-xl border border-[#A855F7]/30 bg-[var(--theme-input-bg)] p-3.5 mb-3">
                                        <div className="flex items-start justify-between">
                                            <div>
                                                ...
                                                <p className="text-[12px] font-semibold text-[#A855F7] mt-0.5">{spill.bountyInfo.budget}</p>
                                            </div>
                                            <span className="text-[9px] font-bold px-2 py-1 rounded-full bg-green-500/10 border border-green-500/20 text-green-500">Active</span>
                                        </div>
                                ...
                                {/* Code block */}
                                {spill.code && (
                                    <div className="rounded-xl bg-[#0D1117] border border-[var(--theme-border-light)] overflow-hidden mb-3">
                                        <div className="flex items-center justify-between px-3 py-1.5 bg-[var(--theme-surface)] border-b border-[var(--theme-border-light)]">
                                            <span className="text-[10px] text-[var(--theme-text-muted)] font-mono">{spill.codeLang}</span>
                                            <button className="text-[9px] text-[var(--theme-text-muted)] hover:text-[#A855F7] cursor-pointer bg-transparent border-none transition-colors">
                                                Copy
                                            </button>
                                        </div>
                                ...
                                {spill.status === "published" && (
                                    <div className="flex items-center justify-between pt-3 border-t border-[var(--theme-border-light)]">
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            <button className="text-[10px] font-medium text-[var(--theme-text-muted)] hover:text-[#A855F7] cursor-pointer bg-transparent border-none transition-colors flex items-center gap-1">
                                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                                                Edit
                                            </button>
                                ...
                                {spill.status === "draft" && (
                                    <div className="flex items-center gap-2 pt-3 border-t border-[var(--theme-border-light)]">
                                        ...
                                        <button className="px-3 py-2 rounded-xl text-[11px] font-medium border border-[var(--theme-border)] bg-[var(--theme-card)] text-red-500 hover:text-red-400 cursor-pointer transition-all ml-auto">
                                            Delete
                                        </button>
                                    </div>
                                )}
