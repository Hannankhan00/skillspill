        ...
        <div style={{ background: "var(--theme-bg)" }} className="min-h-full">
            <div className="max-w-[800px] mx-auto px-4 sm:px-6 py-4 pb-24 lg:pb-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 sm:mb-6">
                    <div>
                        <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-(--theme-text-primary) fade-up">Notifications</h1>
                        <p className="text-[12px] sm:text-[13px] text-(--theme-text-muted) mt-0.5 sm:mt-1">Stay updated on your jobs, matches, and network.</p>
                    </div>
                </div>

                {/* TABS & ACTIONS */}
                <div className="flex items-center justify-between mb-4 border-b border-(--theme-border) pb-0">
                            ...
                            return (
                                <button key={tab} onClick={() => setActiveTab(tab)}
                                    className={`px-3 sm:px-5 py-2.5 sm:py-3 text-[11px] sm:text-[13px] font-semibold border-b-2 transition-all cursor-pointer bg-transparent whitespace-nowrap flex items-center gap-1.5
                                        ${activeTab === tab ? "border-purple-500 text-[#A855F7]" : "border-transparent text-(--theme-text-muted) hover:text-(--theme-text-tertiary)"}`}>
                                    {tab}
                                    <span className={`text-[8px] sm:text-[9px] px-1.5 py-0.5 rounded-full font-bold
                                        ${activeTab === tab ? "bg-[#A855F7]/20 text-[#A855F7]" : "bg-(--theme-input-bg) text-(--theme-text-muted)"}`}>
                                        {count}
                                    </span>
                                </button>
                            );
                    ...
                    {unreadCount > 0 && (
                        <button onClick={markAllRead} className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-[9px] sm:text-[11px] font-bold bg-(--theme-input-bg) text-(--theme-text-secondary) border border-(--theme-border-light) hover:bg-(--theme-bg-secondary) hover:text-(--theme-text-primary) transition-all cursor-pointer shrink-0 mb-1">
                            Mark Read
                        </button>
                    )}
                </div>
                ...
                <div className="space-y-3 sm:space-y-4">
                    {filteredNotifs.map(notif => (
                        <div key={notif.id}
                            className={`rounded-2xl border bg-(--theme-card) shadow-sm p-4 sm:p-5 flex flex-col sm:flex-row gap-3 sm:gap-4 transition-all hover:bg-(--theme-bg-secondary) relative
                            ${!notif.read ? "border-[#A855F7]/40 ring-1 ring-[#A855F7]/20" : "border-(--theme-border)"}`}>
                            ...
                            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-[16px] sm:text-[20px] shrink-0 font-bold bg-(--theme-input-bg) border border-(--theme-border-light) shadow-inner"
                            ...
                            <div className="flex-1 min-w-0 pr-6 sm:pr-8">
                                <div className="flex items-center gap-2 mb-1 sm:mb-1.5">
                                    <p className={`text-[13px] sm:text-[15px] font-bold ${!notif.read ? 'text-(--theme-text-primary)' : 'text-(--theme-text-secondary)'}`}>
                                        {notif.title}
                                    </p>
                                </div>
                                <p className={`text-[12px] sm:text-[13px] leading-relaxed mb-2 sm:mb-3 
                                    ${!notif.read ? 'text-(--theme-text-secondary) font-medium' : 'text-(--theme-text-muted)'}`}>
                                    {notif.message}
                                </p>

                                <div className="flex flex-wrap items-center gap-3 sm:gap-4 mt-2">
                                    <span className="text-[10px] sm:text-[11px] font-bold text-(--theme-text-tertiary) uppercase tracking-widest">{notif.time}</span>
                                    ...
                                    <button onClick={() => toggleRead(notif.id)} className="text-[10px] sm:text-[11px] font-medium text-(--theme-text-muted) hover:text-(--theme-text-primary) cursor-pointer bg-transparent border-none ml-auto sm:ml-0">
                    ...
                    {filteredNotifs.length === 0 && (
                        <div className="py-12 sm:py-16 text-center rounded-2xl border-2 border-dashed border-(--theme-border)">
                            ...
                            <p className="text-[13px] sm:text-[15px] font-bold text-(--theme-text-secondary)">All caught up!</p>
                            <p className="text-[11px] sm:text-[12px] mt-1 text-(--theme-text-muted)">You have no {activeTab === "Unread" ? "unread " : ""}notifications.</p>
                        </div>
                    )}
                </div>
