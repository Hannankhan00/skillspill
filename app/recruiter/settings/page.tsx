...
export default function RecruiterSettingsPage() {
    ...
    useEffect(() => {
        fetch("/api/user/profile")
            ...
            .then(d => {
                ...
                setLinkedinInput(rp.linkedinUrl || "");
    ...
    const saveLinkedin = async (url: string) => {
        setLinkedinSaving(true);
        setLinkedinMsg(null);
        const res = await fetch("/api/user/profile", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ recruiterProfile: { linkedinUrl: url || null } }),
        });
        setLinkedinSaving(false);
        if (res.ok) {
            setLinkedinMsg({ type: "ok", text: url ? "LinkedIn connected!" : "LinkedIn removed." });
        } else {
            setLinkedinMsg({ type: "err", text: "Failed to save. Try again." });
        }
        setTimeout(() => setLinkedinMsg(null), 3000);
    };
    ...
    const [linkedinMsg, setLinkedinMsg] = useState<{ type: "ok" | "err"; text: string } | null>(null);
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="rounded-2xl border overflow-hidden" style={{ background: 'var(--theme-card)', borderColor: 'var(--theme-border)' }}>
                            ...
                            {activeTab === "connections" && (
                                <div className="p-6 space-y-6">
                                    <Section title="Connections" desc="Link your LinkedIn profile to your SkillSpill company account">
                                        {linkedinMsg && (
                                            <div className="rounded-xl px-4 py-3 text-[12px] font-medium"
                                                style={{
                                        ...
                                        <div className="rounded-xl p-4 space-y-3" style={{ background: 'var(--theme-bg)', border: '1px solid var(--theme-border)' }}>
                                            <div className="flex items-center gap-3 mb-1">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[11px]" style={{ color: linkedinInput.trim() ? '#3CF91A' : 'var(--theme-text-muted)' }}>
                                                        {linkedinInput.trim() ? "Connected" : "Not connected"}
                                                    </p>
                                                </div>
                                            </div>
                                            <input
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                {linkedinInput.trim() && (
                                                    <button
                                                        onClick={() => { setLinkedinInput(""); saveLinkedin(""); }}
                                                        disabled={linkedinSaving}
                                                        className="px-4 py-2 rounded-xl text-[11px] font-bold border-none cursor-pointer transition-all disabled:opacity-50"
