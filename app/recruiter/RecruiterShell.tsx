...
export default function RecruiterShell({
    ...
    const initials = companyName ? companyName.substring(0, 2).toUpperCase() : userId.slice(-2).toUpperCase();
            ...
            <aside className="hidden lg:flex w-55 flex-col shrink-0" style={{ background: 'var(--theme-surface)', borderRight: '1px solid var(--theme-border)' }}>
                ...
                <div className="p-4 space-y-3" style={{ borderTop: '1px solid var(--theme-border-light)' }}>
                    <div className="flex items-center gap-3">
                        {userData?.avatarUrl ? (
                            <img src={userData.avatarUrl} alt={companyName} className="w-9 h-9 rounded-full object-cover shrink-0" style={{ border: `1px solid ${accent}40` }} />
                        ) : (
                            <div
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <header className="shrink-0 h-12 flex items-center gap-3 px-3 lg:hidden" style={{ background: 'var(--theme-surface)', borderBottom: '1px solid var(--theme-border)' }}>
                    ...
                    <Link href="/recruiter/profile" className="shrink-0">
                        {userData?.avatarUrl ? (
                            <img src={userData.avatarUrl} alt={companyName} className="w-8 h-8 rounded-full object-cover shadow-sm border-2" style={{ borderColor: `${accent}40` }} />
                        ) : (
                            <div
                ...
                <header className="shrink-0 h-14 hidden lg:flex items-center justify-between px-6 backdrop-blur-xl z-30" style={{ background: 'var(--theme-header-bg)', borderBottom: '1px solid var(--theme-border)' }}>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <div className="relative" ref={dropdownRef}>
                            ...
                            >
                                {userData?.avatarUrl ? (
                                    <img src={userData.avatarUrl} alt={companyName} className="w-8 h-8 rounded-full object-cover shadow-sm border border-black/10" />
                                ) : (
                                ...
                                <div className="text-left">
                                    <p className="text-[11px] font-semibold leading-tight" style={{ color: 'var(--theme-text-secondary)' }}>{companyName}</p>
                                    <p className="text-[9px] leading-tight" style={{ color: 'var(--theme-text-muted)' }}>{jobTitle}</p>
                                </div>
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2">
                            ...
                            {profileDropdown && (
                                <div className="absolute right-0 top-12 w-52 rounded-xl shadow-xl py-1.5 z-50" style={{ background: 'var(--theme-surface)', border: '1px solid var(--theme-border)' }}>
                                    <div className="px-4 py-3" style={{ borderBottom: '1px solid var(--theme-border-light)' }}>
                                        <div className="flex items-center gap-3">
                                            {userData?.avatarUrl ? (
                                                <img src={userData.avatarUrl} alt={companyName} className="w-9 h-9 rounded-full object-cover border border-black/10" />
                                            ) : (
                                            ...
                                            <div>
                                                <p className="text-[12px] font-semibold" style={{ color: 'var(--theme-text-primary)' }}>{companyName}</p>
                                                <p className="text-[10px]" style={{ color: 'var(--theme-text-muted)' }}>{jobTitle}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="py-1">
                                        ...
                                        <button onClick={() => { setProfileDropdown(false); setNotifPanelOpen(true); }}
                                            className="flex items-center gap-3 px-4 py-2.5 text-[12px] hover:text-[#A855F7] transition-colors w-full text-left bg-transparent border-none cursor-pointer" style={{ color: 'var(--theme-text-tertiary)' }}>
                                            ...
                                            Notifications
                                            {unreadCount > 0 && <span className="ml-auto text-[9px] px-1.5 py-0.5 rounded-full bg-red-100 text-red-600 font-bold">{unreadCount > 9 ? "9+" : unreadCount}</span>}
                                        </button>
                                    </div>
                            ...
                            return (
                                <button key={item.label} onClick={() => setNotifPanelOpen(true)}
                                    className={`relative flex flex-col items-center justify-center gap-0.5 py-1 px-2 transition-colors bg-transparent border-none cursor-pointer
                                        ${notifPanelOpen ? "text-[#A855F7]" : "text-(--theme-text-muted)"}`}>
