...
const TYPE_META: Record<string, { icon: React.ReactNode; color: string; bg: string }> = {
    ...
    match:       { icon: <Sparkles className="w-5 h-5" />,        color: "#3CF91A", bg: "#3CF91A20" },
            ...
            <div className="max-w-xl mx-auto px-0 sm:px-4 py-0 pb-24 lg:pb-8">
                    ...
                    <div className="flex gap-2">
                        {(["all", "unread"] as const).map(f => (
                            <button
                                ...
                                style={filter === f
                                    ? { background: accentDark, color: "#fff", borderColor: accentDark }
                ...
                {loading ? (
                    <div className="flex items-center justify-center py-24">
                        <Loader2 className="w-7 h-7 animate-spin" style={{ color: accentDark }} />
                    </div>
                ) : visible.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24 px-6 text-center">
                        <div className="w-16 h-16 rounded-full flex items-center justify-center mb-4" style={{ background: `${accent}20` }}>
                            <Bell className="w-7 h-7" style={{ color: accentDark }} />
                        </div>
                                ...
                                {group.items.map(n => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            {/* Unread left stripe */}
                                            {!n.isRead && (
                                                <div className="absolute left-0 top-0 bottom-0 w-0.75 rounded-r-full" style={{ background: accentDark }} />
                                            )}
                                            ...
                                            <Wrapper
                                                ...
                                                <div className="flex-1 min-w-0 pr-8">
                                                    ...
                                                    <p className="text-[11px] mt-1 font-medium" style={{ color: !n.isRead ? accentDark : "var(--theme-text-muted)" }}>
                                                        {timeAgo(n.createdAt)}
                                                    </p>
                                                </div>
                                                ...
                                                {/* Unread dot */}
                                                {!n.isRead && (
                                                    <div className="absolute right-4 top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full shrink-0" style={{ background: accentDark }} />
                                                )}
                                            </Wrapper>
