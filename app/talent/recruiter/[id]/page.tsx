    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-full p-20">
                ...
                <p className="text-[13px] text-(--theme-text-muted) font-medium">Loading recruiter profile...</p>
            </div>
        );
    }
    ...
    if (!recruiterData) {
        return (
            <div className="flex flex-col items-center justify-center min-h-full p-20">
                ...
                <p className="text-[13px] text-(--theme-text-muted)">We couldn&apos;t locate this recruiter&apos;s profile.</p>
            </div>
        );
    }

    const { fullName, username, recruiterProfile, spills, avatarUrl, coverUrl, _count } = recruiterData;
    ...
    return (
        <div style={{ background: "var(--theme-bg)" }} className="min-h-full">
            ...
            <div className="relative">
                <div className={`h-32 sm:h-44 lg:h-52 w-full relative overflow-hidden ${coverUrl || "bg-linear-to-r from-violet-600 via-purple-600 to-indigo-600"}`}>
                ...
                <div className="max-w-[900px] mx-auto px-4 sm:px-6">
                    <div className="relative -mt-12 sm:-mt-16 flex items-end gap-4">
                        <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full overflow-hidden bg-linear-to-br from-violet-400 to-purple-600 flex items-center justify-center text-white text-2xl sm:text-3xl font-bold border-4 border-(--theme-bg) shadow-xl shrink-0">
                            {avatarUrl ? (
            ...
            <div className="max-w-[900px] mx-auto px-4 sm:px-6 pb-24 lg:pb-8">
                ...
                <div className="flex items-center gap-3 sm:gap-6 mt-4 sm:mt-5 pb-4 border-b border-(--theme-border)">
                    <div className="text-center">
                        ...
                        <p className="text-[9px] sm:text-[10px] text-(--theme-text-muted) uppercase tracking-wider">Open Jobs</p>
                    </div>
                    <div className="text-center">
                    ...
                    {activeTab === "Overview" && (
                        <>
                            ...
                            <div className="rounded-2xl border border-(--theme-border) bg-(--theme-card) shadow-sm p-4 sm:p-5">
                                ...
                                <p className="text-[13px] text-(--theme-text-tertiary) leading-relaxed whitespace-pre-wrap">
                            ...
                            {companyWebsite && (
                                <div className="rounded-2xl border border-(--theme-border) bg-(--theme-card) shadow-sm p-4 sm:p-5">
                                    ...
                                    <a href={companyWebsite.startsWith("http") ? companyWebsite : `https://${companyWebsite}`}
                                        ...
                                        <div>
                                            <p className="text-[12px] font-bold text-(--theme-text-primary)">Company Website</p>
                                            <p className="text-[10px] text-(--theme-text-muted)">{companyWebsite.replace(/^https?:\/\//, "")}</p>
                                        </div>
                                    </a>
                                </div>
                            )}
                        ...
                        <div className="space-y-4">
                            {bounties && bounties.length > 0 ? (
                                ...
                                <div className="rounded-2xl border border-(--theme-border) bg-(--theme-card) shadow-sm p-8 text-center flex flex-col items-center">
                                    ...
                                    <h3 className="text-[14px] font-bold text-(--theme-text-primary) mb-1">No Open Jobs</h3>
                                    <p className="text-[12px] text-(--theme-text-muted)">This recruiter has no active bounties right now.</p>
                                </div>
                            )}
                        </div>
                    )}
                            ...
                            {spills && spills.length > 0 ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {spills.map((spill: any) => (
                                        <div key={spill.id}
                                            className="rounded-2xl border border-(--theme-border) bg-(--theme-card) shadow-sm overflow-hidden flex flex-col h-full transition-all"
                                            onMouseEnter={e => (e.currentTarget.style.borderColor = `${accent}40`)}
                                            onMouseLeave={e => (e.currentTarget.style.borderColor = "var(--theme-border)")}>
                                            <div className="p-4 sm:p-5 flex-1">
                                                <div className="flex items-center gap-3 mb-3">
                                                    <div className="w-8 h-8 rounded-full bg-linear-to-br from-violet-400 to-purple-600 flex items-center justify-center text-white text-[9px] font-bold shrink-0">
                                                        {initials}
                                                    </div>
                                                    <div>
                                                ...
                                                {spill.code && (
                                                    <div className="rounded-xl bg-[#0D1117] border border-(--theme-code-border) overflow-hidden mb-3">
                                                        <pre className="px-3 py-3 text-[10px] text-green-400 font-mono overflow-hidden h-20 relative" style={{ margin: 0 }}>
                                                            ...
                                                            <div className="absolute bottom-0 left-0 right-0 h-10 bg-linear-to-t from-[#0D1117] to-transparent pointer-events-none" />
                                                        </pre>
                                                    </div>
                                                )}
                                                {spill.tags && (
                                            ...
                                            <div className="px-4 py-3 border-t border-(--theme-border-light) flex flex-wrap items-center gap-4 text-[11px] text-(--theme-text-muted)"
                                ...
                                <div className="rounded-2xl border border-(--theme-border) bg-(--theme-card) shadow-sm p-8 text-center flex flex-col items-center">
                                    ...
                                    <p className="text-[12px] text-(--theme-text-muted)">This recruiter hasn&apos;t posted any spills.</p>
                                </div>
                            )}
                        </div>
                    )}
