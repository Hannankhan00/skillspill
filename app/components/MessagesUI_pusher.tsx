...
// ÔöÇÔöÇ Tick icons ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
// Single grey tick  ÔåÆ sent (not yet delivered/read)
// Double grey ticks ÔåÆ delivered
// Double green ticks ÔåÆ read
...
export default function MessagesUI({ accent }: { accent: string }) {
    ...
    // ÔöÇÔöÇ Mobile keyboard: use visualViewport to shrink the chat area ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    useEffect(() => {
    ...
    // Shared message handler ÔÇö used by both the conversation channel and the personal user channel
    const handleIncomingMessage = useCallback((msg: Message) => {
        ...
        // Always update the sidebar ÔÇö unread count + last message preview + re-sort to top
        setConversations(prev => {
            const exists = prev.some(c => c.id === msg.conversationId);
            if (!exists) {
                // First-ever message from this person ÔÇö reload full list for complete metadata
    ...
    useEffect(() => {
        fetch("/api/user/profile")
            ...
            .then(d => {
                ...
                // Personal channel ÔÇö receives new messages for ANY conversation,
    ...
    // ÔöÇÔöÇ Conversation switch: load messages + subscribe to Pusher channel ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    useEffect(() => {
        if (!activeConvoId) {
            setMessages([]);
    ...
    // ÔöÇÔöÇ Send message ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    const sendMessage = async () => {
        ...
        // Optimistic ÔÇö id starts with "tmp-" so we know it's not real
    ...
    return (
        ...
        >
            {/* ÔöÇÔöÇ SIDEBAR ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
            <div
            ...
            >
                <div className="p-4 border-b border-(--theme-border) shrink-0">
                    ...
                    <div className="mt-2.5 relative">
                        ...
                        <input type="text" placeholder="Search conversationsÔÇª"
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-1">
                                        ...
                                        <p className={`text-[11px] truncate ${c.unread > 0 ? "font-semibold text-(--theme-text-primary)" : "text-(--theme-text-muted)"}`}>
                                            {c.lastMessage ?? "Say hello ­ƒæï"}
                                        </p>
                                    </div>
                                    <p className="text-[10px] mt-0.5 font-medium" style={{ color: `${accent}99` }}>
                                        @{c.other.username} ┬À {c.other.role.toLowerCase()}
                                    </p>
                                </div>
                            </button>
                        ))
            ...
            {/* ÔöÇÔöÇ CHAT AREA ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
            <div
            ...
            >
                {activeConvo ? (
                    <>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <p className="text-[10px] text-(--theme-text-muted)">
                                            {typingUsers.size > 0 ? "TypingÔÇª" : pusherConnected ? "Online" : "ConnectingÔÇª"}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        >
                            {loadingMessages ? (
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-center pb-8">
                                    ...
                                    <p className="text-[11px] text-(--theme-text-muted) mt-1">Say hello ­ƒæï</p>
                                </div>
                            ) : (
                                messages.map((msg, i) => {
                                    ...
                                    return (
                                        <React.Fragment key={msg.id}>
                                            ...
                                            <div className={`flex items-end gap-2 ${isMine ? "justify-end" : "justify-start"} ${sameAsPrev && !showDate ? "mt-0.5" : "mt-2"}`}>
                                                {/* Avatar ÔÇö only show on last message of a group */}
                                                {!isMine && (
                                                ...
                                                <div className={`flex flex-col ${isMine ? "items-end" : "items-start"} max-w-[82%] sm:max-w-[68%]`}>
                                                    ...
                                                    {/* Timestamp + tick ÔÇö only on last msg of group or last overall */}
                        ...
                        {/* Input bar ÔÇö sticky at bottom, moves with keyboard */}
                        <div
                        ...
                        >
                            ...
                            >
                                ...
                                <input
                                    ...
                                    onFocus={() => {
                                        // Small delay ÔÇö let the keyboard appear, then scroll to bottom
