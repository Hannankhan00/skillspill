...
export default function HashtagPage() {
    ...
    const fetchPosts = useCallback(async (reset = false) => {
        ...
        try {
            const params = new URLSearchParams({ limit: "10", hashtag });
    ...
    useEffect(() => { fetchPosts(true); }, [hashtag]);
        ...
        <div className="min-h-screen" style={{ background: "var(--theme-bg)" }}>
            <div className="sticky top-0 z-30 backdrop-blur-xl" style={{ background: "var(--theme-header-bg)", borderBottom: "1px solid var(--theme-border)" }}>
                <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-[15px] font-bold" style={{ color: "var(--theme-text-primary)" }}>#{hashtag}</h1>
                        <p className="text-[10px]" style={{ color: "var(--theme-text-muted)" }}>{posts.length} posts</p>
                    </div>
                </div>
            </div>
